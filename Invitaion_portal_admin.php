	...
	<body>
							...
							<div class='col-sm-12'>
								...
								<form id='eventDetails' method='post' enctype="multipart/form-data" action="">
									...
									<div class='col-sm-12 widgetDescriptionForm'>
										<div class='row'>
											<h4 class='header_title'>Invitation Detail</h4>
											<a href="Invitaion_portal_admin_temp.php">
												<h4 class='template_heading'>Select Template View</h4>
											</a>
											<div class='widgetDescriptionRow clearfix'>
												<fieldset class='col-sm-4 form-group'>
													...
													<input id='invitaionName' name='invitaionName' class='form-control'
												...
												<fieldset class='col-sm-4 form-group'>
													...
													<input id='invitaionFrom' name='invitaionForm' class='form-control'
														onblur='this.value=this.value.trim();' type='text' value=''
														maxlength='30'>
													<div class='discriptionErrorMsg hosterror'>
													</div>
												</fieldset>
												<fieldset class='col-sm-4 form-group'>
													...
													<div id='datetimepicker1' class='input-append date'>
														<input id='eventDate' name='eventDate'
															data-format='dd/MM/yyyy h:i:s a' type='text' readonly></input>
														<span class='add-on'>
												...
												<fieldset class='col-sm-4 form-group'>
													...
													<input id='eventPlace' name='eventPlace' class='form-control'
														onblur='this.value=this.value.trim();' type='text' value=''
														maxlength='30'>
													<div class='discriptionErrorMsg venueerror'>
													</div>
												</fieldset>
												...
												<fieldset class='col-sm-4 form-group'>
													...
													<div class='discriptionErrorMsg messageerror'>
													</div>
												</fieldset>
												<fieldset class='col-sm-4 form-group'>
													<label><span class='requiredFiled'><input type="checkbox"
																name="private_email" id="private_email"
																value="private_email" onchange="myFunction()"></span>Private
														email</label>
													<input type="email" name="privateemailid" id="privateemailid"
														class='form-control'>
												...
												<fieldset class='col-sm-4 form-group'>
													<label><span class='requiredFiled'><input type="checkbox"
																name="dynamic_template" id="dynamic_template"
																value="dynamic_template"
																onchange="gettemplate()"></span>Dynamic Template</label>
													<input type="file" name="dynamic_templateid" id="dynamic_templateid"
														class='form-control'>
											...
											<div class='widgetDescriptionRow clearfix'>
												<div class="col-sm-12">
													...
													?>
														...
														<div class='discriptionErrorMsg inviteerror'>
														</div>
													</fieldset>
													<fieldset class='col-sm-12 form-group clear-both bulk_invitees'>
														<label><span class='requiredFiled'>*</span>Upload Bulk
															Invitees</label>
										...
										<fieldset class='form-group pull-right'>
											...
											<div class='widgetSearchButton no-search-criteria'>
												<input type='submit' value='Send Invitaion' name='submit' id='invitDatetime'
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#eventDetails').on('submit', function() {
					var invitaionName = $('#invitaionName').val();
					var invitaionFrom = $('#invitaionFrom').val();
					var eventDate = $('#eventDate').val();
					var eventPlace = $('#eventPlace').val();
					var selectinvite = $('#test-select').val();
					var address = $('#address').val();
					var Message = $('#Message').val();
					var selected = $('.selected').text().length;
					var emailcheckbox = $('input[name=private_email]').is(':checked');
					var dynamicTemplate = $('input[name=dynamic_template]').is(':checked');
					var email = $('#privateemailid').val();
					var templateid = $('#dynamic_templateid').val();
					...
					$(".addresserror").empty();
					$(".messageerror").empty();
					$(".privateemailiderror").empty();
					$(".dynamictemplateerror").empty();
					$(".inviteerror").empty();
					if (invitaionName.length == 0 && invitaionFrom.length == 0 && eventDate.length == 0 &&
						eventPlace.length == 0 && address.length == 0 && Message.length == 0 && selected == 0) {
						$('.nameerror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation Name field is required</span>'
							);
						$('.hosterror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation Host field is required</span>'
							);
						$('.dateerror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation Date and time field is required</span>'
							);
						$('.venueerror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation venue field is required</span>'
							);
						$('.addresserror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation address field is required</span>'
							);
						$('.messageerror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation message field is required</span>'
							);
						$('.inviteerror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation List field is required</span>'
							);
						if (emailcheckbox && email.length == 0) {
							$('.privateemailiderror').append(
								'<span id="eventNameerror" class="red-color-error">Invitation private email field is required</span>'
								);
						}
						if (dynamicTemplate && templateid.length == 0) {
							$('.dynamictemplateerror').append(
								'<span id="eventNameerror" class="red-color-error">Image is required</span>'
								);
						}
						return false;
					}
					if (invitaionName.length == 0) {
						$(".nameerror").empty();
						$('.nameerror').append(
							'<span id="eventNameerror" class="red-color-error">Invitation Name field is required</span>'
							);
